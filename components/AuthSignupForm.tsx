...
export default function AuthSignupForm() {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!signUpData.session || !signUpData.user) {
        throw new Error(
          "Signup succeeded but no session was created. Disable email confirmation in Supabase Auth settings for demo signups."
        );
      }
      ...
      setSuccess("School login created. Redirecting…");
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      >
        {loading ? "Creating…" : "Create school login"}
      </button>
      <p className="text-center text-sm text-slate-600">
        Already have a school login?{" "}
